...
def prepare_listing_row(row: dict[str, str]) -> tuple[Any, ...]:
    ...
    return (
        ...
        _clean_text(row.get("nearest_stop_name")),
        _parse_float(row.get("nearest_stop_distance_m")),
        _clean_text(row.get("nearest_train_name")),
        _parse_float(row.get("nearest_train_distance_m")),
        _clean_text(row.get("nearest_hb_name")),
        _parse_float(row.get("nearest_hb_distance_m")),
        # --- v3.1: administrative geography ---
        _parse_int(row.get("municipality_code")),
        _parse_int(row.get("district_code")),
        _parse_int(row.get("canton_code")),
        _clean_text(row.get("municipality_name")),
        _clean_text(row.get("district_name")),
        _clean_text(row.get("canton_name")),
        _clean_text(row.get("municipality_name_demo")),
        # --- v3.1: demographics ---
        _parse_int(row.get("population_total")),
        _parse_float(row.get("area_ha")),
        _parse_float(row.get("area_km2")),
        _parse_float(row.get("population_density")),
        _clean_text(row.get("population_density_bucket")),
        # --- v3.1: multi-level price benchmarking ---
        _parse_float(row.get("price_per_m2")),
        _parse_float(row.get("avg_price_per_m2_municipality")),
        _parse_float(row.get("avg_price_per_m2_district")),
        _parse_float(row.get("avg_price_per_m2_canton")),
        _parse_float(row.get("price_per_m2_vs_municipality")),
        _parse_float(row.get("price_per_m2_vs_district")),
        _parse_float(row.get("price_per_m2_vs_canton")),
        _clean_text(row.get("price_per_m2_vs_municipality_label")),
        # --- JSON blobs ---
